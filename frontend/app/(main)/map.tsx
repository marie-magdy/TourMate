...
const WALKABLE_DISTANCE_KM = 1.0;  // 1 km threshold for showing the walkable banner
...
export default function MapScreen() {
  ...
  // ── Walking-in-progress state (GPS verification) ─────────────────
  const [walkingInProgress, setWalkingInProgress] = useState(false);
  const [walkTarget, setWalkTarget] = useState<Place | null>(null);
  const [walkDistanceLeft, setWalkDistanceLeft] = useState<number>(0); // metres
  const [vehicleDetected, setVehicleDetected] = useState(false); // true = cheated with a vehicle
  const locationWatcherRef = useRef<Location.LocationSubscription | null>(null);
  const walkInitialDistRef = useRef<number>(1); // metres — set when walk begins

  // Stop watcher on unmount to avoid memory leaks
    ...
    (async () => {
      try {
        ...
        if (raw) {
          ...
          if (p.locationServices !== undefined) setLocationEnabled(!!p.locationServices);
        }
      } catch {}
    })();
  }, []);

  useEffect(() => { getUserLocation(); }, [locationEnabled]);
  ...
  // ── Stop any running location watcher ────────────────────────────
  ...
  // ── Cancel an in-progress walk (no points awarded) ────────────────
  const cancelWalk = () => {
    stopLocationWatcher();
  ...
  // ── Award points to the real logged-in user ───────────────────────
  const awardWalkPoints = async (place: Place) => {
    try {
      // Resolve the real user ID — context first, AsyncStorage as fallback
      let resolvedId: number | string = userId;
      if (!resolvedId) {
